...
class Evaluator(object):
    def __init__(self, args):
        ...
        self.model = get_segmentation_model(model=args.model, dataset=args.dataset, backbone=args.backbone,
                                            ...
                                            #local_rank=self.device, ##xiaoxuan
    ...
    def eval(self):
        ...
        for i, (image, target, filename) in enumerate(self.val_loader):
            ...
            if self.args.save_pred:
                ...
                print(predict.shape)
                #cv2.imwrite(os.path.join(outdir, os.path.splitext(filename[0])[0] + '.png') , predict)
                mask = get_color_pallete(predict, self.args.dataset)
                mask.save(os.path.join(outdir, os.path.splitext(filename[0])[0] + '.png'))
        end = time.time()
        fps = len(self.val_loader)/(end-start_time)
        print(fps)
        logger.info("fps: {:f}".format(fps))
        synchronize()
