...
from core.utils.ms_ssim_l1 import MS_SSIM_L1_LOSS
...
def parse_args():
    ...
    # model and dataset
    parser.add_argument('--model', type=str, default='unet3plus',
                        choices=['fcn32s', 'fcn16s', 'fcn8s',
                                 'fcn', 'psp', 'deeplabv3', 'deeplabv3_plus',
                                 'danet', 'denseaspp', 'bisenet', 'bisenetv1', 'bisenetv2',
                                 'encnet', 'dunet', 'icnet',
                                 'enet', 'ocnet', 'ccnet', 'psanet',
                                 'cgnet', 'espnet', 'lednet', 'dfanet',
                                 'unet','unet2plus','unet3plus',
                                 'attunet','r2unet','r2attunet','segnet'],
                        help='model name (default: fcn32s)')
    parser.add_argument('--backbone', type=str, default='resnet50',
                        choices=['vgg16', 'resnet18', 'resnet50',
                                 'resnet101', 'resnet152', 'xception','densenet121',
    ...
    parser.add_argument('--crop-size', type=int, default=416,#480
                        help='crop image size')
    parser.add_argument('--workers', '-j', type=int, default=0,  #4
                        metavar='N', help='dataloader threads')
    # training hyper params
    parser.add_argument('--jpu', action='store_true', default=False,
                        help='JPU')
    parser.add_argument('--use-ohem', type=bool, default=False,  #False
                        help='OHEM Loss for cityscapes dataset')
    parser.add_argument('--aux', action='store_true', default=False,
                        help='Auxiliary loss')
    parser.add_argument('--aux-weight', type=float, default=0.4,
                        help='auxiliary loss weight')
    parser.add_argument('--batch-size', type=int, default=24, metavar='N', ##4
    ...
    parser.add_argument('--save-dir', default='../runs/models',         #~/.torch/models
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.model = get_segmentation_model(model=args.model, dataset=args.dataset, backbone=args.backbone,
                                            pretrained_base=False,     ##xiaoxuan no-pretrain
        ...
        self.criterion = get_segmentation_loss(args.model, use_ohem=args.use_ohem, aux=args.aux,
                                               aux_weight=args.aux_weight, ignore_index=-1).to(self.device)  #ignore_index = -1
    ...
    def train(self):
        ...
        for iteration, (images, targets, _) in enumerate(self.train_loader):
            ...
            #print(images.shape) #4,3,384,672
    ...
    def validation(self):
        ...
        model.eval()
        #total = sum([param.nelement() for param in self.model.parameters()])
        #print("Number of parameter of validation: %.2fM" % (total / 1e6))
