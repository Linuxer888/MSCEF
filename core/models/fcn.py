...
class FCN32s(nn.Module):
    ...
    def __init__(self, nclass, backbone='vgg16', aux=False, pretrained_base=True,
        ...
        #self.__setattr__('exclusive', ['pretrained', 'head', 'auxlayer'] if aux else ['pretrained', 'head'])
...
class FCN8s(nn.Module):
    def __init__(self, nclass=2, backbone='vgg16', aux=False, pretrained_base=False, norm_layer=nn.BatchNorm2d, **kwargs):
        super(FCN8s, self).__init__()
        self.aux = aux
        if backbone == 'vgg16':
            self.pretrained = vgg16(pretrained=pretrained_base).features  ##pretrained
        ...
        '''
        self.__setattr__('exclusive',
                         ['pool3', 'pool4', 'pool5', 'head', 'score_pool3', 'score_pool4', 'auxlayer'] if aux else ['pool3', 'pool4', 'pool5', 'head', 'score_pool3',
                         'score_pool4'])
        '''
...
class _FCNHead(nn.Module):
    def __init__(self, in_channels, channels, norm_layer=nn.BatchNorm2d, **kwargs):
        ...
        #inter_channels = in_channels //5  #xiaoxuan
...
def get_fcn32s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='../runs/models',  #~/.torch/models
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('fcn32s_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model


def get_fcn16s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='../runs/models',
               pretrained_base=True, **kwargs):
    acronyms = {
        'pascal_voc': 'pascal_voc',
        'pascal_aug': 'pascal_aug',
    ...
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('fcn16s_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model


def get_fcn8s(dataset='pascal_voc', backbone='vgg16', pretrained=False, root='../runs/models',
              pretrained_base=True, **kwargs):
    acronyms = {
        'pascal_voc': 'pascal_voc',
        'pascal_aug': 'pascal_aug',
    ...
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('fcn8s_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
