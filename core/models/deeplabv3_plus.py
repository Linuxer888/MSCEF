...
class DeepLabV3Plus(nn.Module):
    ...
    def __init__(self, nclass, backbone='xception', aux=False, pretrained_base=True, dilated=True, **kwargs):
...
def get_deeplabv3_plus(dataset='pascal_voc', backbone='xception', pretrained=False, root='../runs/models',
                       pretrained_base=True, **kwargs):    # pretrained_base=True
    acronyms = {
    ...
    model = DeepLabV3Plus(datasets[dataset].NUM_CLASS, backbone=backbone, pretrained_base=False, **kwargs)#pretrained_base=pretrained_base
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(
            torch.load(get_model_file('deeplabv3_plus_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                map_location=device))
    return model
