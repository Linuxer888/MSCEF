...
def get_denseaspp(dataset='citys', backbone='densenet121', pretrained=False,
                  root='../runs/models', pretrained_base=True, **kwargs): #~/.torch/models
    r"""DenseASPP
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('denseaspp_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
