...
class ICNet(SegBaseModel):
    ...
    def __init__(self, nclass, backbone='resnet50', aux=False, jpu=False, pretrained_base=True, **kwargs):
        ...
        #self.hhh = _ConvBNReLU(100,32,3,1,**kwargs) #xiaoxuan
...
def get_icnet(dataset='citys', backbone='resnet50', pretrained=False, root='../runs/models',
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('icnet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
def get_icnet_resnet50_laser(**kwargs):
    return get_icnet('laser', 'resnet50', **kwargs)


def get_icnet_resnet101_laser(**kwargs):
    return get_icnet('laser', 'resnet101', **kwargs)
