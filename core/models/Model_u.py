...
def get_r2unet(dataset='pascal_voc', backbone='', pretrained=False, root='../runs/models',
    ...
    if pretrained:
        ...
        model.load_state_dict(
            torch.load(get_model_file('r2unet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                       map_location=device))
    return model
...
def get_AttU_Net(dataset='pascal_voc', backbone='', pretrained=False, root='../runs/models',
    ...
    if pretrained:
        ...
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(
            torch.load(get_model_file('attunet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                       map_location=device))
    return model
...
def get_r2attunet(dataset='pascal_voc', backbone='', pretrained=False, root='../runs/models',
    ...
    if pretrained:
        ...
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(
            torch.load(get_model_file('r2attunet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                       map_location=device))
    return model
