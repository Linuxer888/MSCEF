...
from core.nn.basic import _ConvBNReLU
...
class BiSeNet(nn.Module):
    def __init__(self, nclass, backbone='resnet18', aux=False, jpu=False, pretrained_base=True, **kwargs):
        ...
        if aux:
            self.auxlayer1 = _BiSeHead(128, 26, nclass, **kwargs)
            self.auxlayer2 = _BiSeHead(128, 256, nclass, **kwargs)

        self.__setattr__('exclusive',
                         ['spatial_path', 'head',
                          'context_path', 'ffm',  'auxlayer1', 'auxlayer2'
                          ] if aux else [
                             'spatial_path',
                             'context_path', 'ffm',
                             'head'])
...
class ContextPath(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.layer2(x)
        #print('layers2:{}'.format(x.size()))
        context_blocks.append(x)
        c3 = self.layer3(x)
        #print('layers3:{}'.format(c3.size()))
        context_blocks.append(c3)
        c4 = self.layer4(c3)
        #print('layers4:{}'.format(c4.size()))
...
def get_bisenet(dataset='citys', backbone='resnet18', pretrained=False, root='../runs/models',
...
if __name__ == '__main__':
    img = torch.randn(4, 3, 400, 680)
    model = BiSeNet(2, backbone='resnet18', pretrained_base=False)
    model.cuda()
    model.eval()
    model(img)
