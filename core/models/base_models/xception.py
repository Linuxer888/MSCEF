...
# Constructor
def get_xception(pretrained=False, root='../datasets/models', **kwargs):
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('Xception'), root=root), map_location=device)
    return model
...
def get_xception_a(pretrained=False, root='../runs/models', **kwargs):
    model = XceptionA()##hanmh
    if pretrained:
        from ..model_store import get_model_file
