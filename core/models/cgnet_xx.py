...
class conv_block_de(nn.Module):#conv_block_de
    """
    Convolution Block
    """
    ...
    def __init__(self, in_ch, out_ch):
        ...
        self.conv01 = nn.Conv2d(in_ch, out_ch, 3,groups=in_ch, padding=1)
            #nn.Sequential(
            #nn.Conv2d(in_ch, out_ch, 3,groups=in_ch, padding=1),
            #nn.BatchNorm2d(out_ch//4),
            #nn.ReLU(inplace=True)
        #)

        self.conv02 = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, (1, 3),groups=in_ch, padding=(0, 2), dilation=2),
            nn.Conv2d(out_ch, out_ch, (7, 1), groups=out_ch,padding=(6, 0), dilation=2),
        )

        self.conv03 = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, (3, 1),groups=in_ch, padding=(2, 0), dilation=2),
            nn.Conv2d(out_ch, out_ch, (1, 7),groups=out_ch, padding=(0,6), dilation=2),
        )

        self.conv04 = nn.Conv2d(in_ch, out_ch, 3, padding=2,groups=in_ch, dilation=2)
            #nn.Sequential(
            #nn.Conv2d(in_ch, out_ch, 3, padding=2,groups=in_ch, dilation=2),
            #nn.BatchNorm2d(out_ch // 4),
            #nn.ReLU(inplace=True),
        #)
...
class ContextGuidedBlock(nn.Module):
    def __init__(self, in_channels, out_channels, dilation=2, reduction=16, down=False,
        ...
        inter_channels = out_channels // 4 if not down else out_channels  #
        if down:
            self.conv = _ConvBNPReLU(in_channels, inter_channels, 3, 2, 1, norm_layer=norm_layer, **kwargs)
            self.reduce = nn.Conv2d(inter_channels * 4, out_channels, 1, bias=False)#*2
        else:
        ...
        self.f = conv_block_de(inter_channels, inter_channels)
        self.bn = norm_layer(inter_channels * 4) #*2
        self.prelu = nn.PReLU(inter_channels * 4)
...
def get_cgnet(dataset='citys', backbone='', pretrained=False, root='../runs/models', pretrained_base=True, **kwargs):
    acronyms = {
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('cgnet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
def get_cgnet_laser(**kwargs):
    return get_cgnet('laser', '', **kwargs)
