...
class ESPNetV2(nn.Module):
    ...
    def __init__(self, nclass, backbone='', aux=False, jpu=False, pretrained_base=False, **kwargs):
        ...
        self.pspMod = nn.Sequential(
            ...
            EESP(256, 128, stride=1, k=4, r_lim=7), #xiaoxuan
...
def get_espnet(dataset='pascal_voc', backbone='', pretrained=False, root='../runs/models',
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('espnet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
def get_espnet_laser(**kwargs):
    return get_espnet('laser', **kwargs)
