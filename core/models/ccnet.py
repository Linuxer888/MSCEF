...
def get_ccnet(dataset='pascal_voc', backbone='resnet50', pretrained=False, root='../runs/models',
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('ccnet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
def get_ccnet_resnet50_laser(**kwargs):
    return get_ccnet('laser', 'resnet50', **kwargs)
