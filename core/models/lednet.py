...
class Decoder(nn.Module):
    ...
    def forward(self, input):
        ...
        #print(output.shape)
        out = interpolate(output, size=(672, 800), mode="bilinear", align_corners=True)
...
class Net(nn.Module):
    ...
    def forward(self, input, only_encode=False):
        if only_encode:
            ...
            outputs = list()
            outputs.append(self.decoder.forward(output))
            return tuple(outputs)

def get_lednet(dataset='citys', backbone='', pretrained=False, root='../runs/models',
    ...
    model = Net(datasets[dataset].NUM_CLASS)
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('lednet_%s_%s_best_model' % (backbone,acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
def get_lednet_laser(**kwargs):
    return get_lednet('laser', **kwargs)