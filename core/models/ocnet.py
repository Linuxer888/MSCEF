...
def get_ocnet(dataset='citys', backbone='resnet50', oc_arch='base', pretrained=False, root='../runs/models',
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('ocnet_%s_%s_best_model' % (
            backbone, acronyms[dataset]), root=root),
            map_location=device))
    return model
