...
def get_dunet(dataset='pascal_voc', backbone='resnet50', pretrained=False,
              root='../runs/models', pretrained_base=True, **kwargs):
    acronyms = {
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('dunet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
def get_dunet_resnet50_laser(**kwargs):
    return get_dunet('laser', 'resnet50', **kwargs)


def get_dunet_resnet101_laser(**kwargs):
    return get_dunet('laser', 'resnet101', **kwargs)


def get_dunet_resnet152_pascal_voc(**kwargs):
    return get_dunet('laser', 'resnet152', **kwargs)
