...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, inter_channels, out_channels, dilation=1, asymmetric=False,
        ...
        if downsampling:
            ...
            #'''
            #self.conv2 = conv_block_de(inter_channels,inter_channels)
...
def get_enet(dataset='citys', backbone='', pretrained=False, root='../runs/models', pretrained_base=True, **kwargs):
    acronyms = {
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('enet_%s_%s_best_model' % (backbone,acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
def get_enet_laser(**kwargs):
    return get_enet('laser', '', **kwargs)
