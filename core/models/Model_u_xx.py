...
class conv_block_xx(nn.Module):
    ...
    def __init__(self, in_ch, out_ch):
        ...
        #self.att1 = SpatialAttention_xx(out_ch//4)
        #'''
        ...
        #self.att2 = SpatialAttention_xx(out_ch // 4)
        ...
        #self.att3 = SpatialAttention_xx(out_ch // 4)
        '''
        ...
        self.conv02 = nn.Sequential(
            nn.Conv2d(out_ch, out_ch // 4, 3, padding=3, dilation=3),
            nn.BatchNorm2d(out_ch // 4),
            nn.ReLU(inplace=True),
        )
        self.conv03 = nn.Sequential(
            nn.Conv2d(out_ch, out_ch // 4, 3, padding=4, dilation=4),
            nn.BatchNorm2d(out_ch // 4),
            nn.ReLU(inplace=True),
        )
        ...
        #self.att4 = SpatialAttention_xx(out_ch // 4)
        ...
        #self.att1 = SpatialAttention_xx(out_ch)
...
class R2U_Net(nn.Module):
    ...
    """
    def __init__(self,output_ch=1, img_ch=3,  t=2):
        ...
        '''
        self.__setattr__('exclusive', ['Maxpool1', 'Maxpool2', 'Maxpool3', 'Maxpool',
                                       'Upsample', 'RRCNN1', 'RRCNN2', 'RRCNN3', 'RRCNN4','RRCNN5',
                                       'Up5',  'Up_RRCNN5', 'Up4', 'Up_RRCNN4',
                                       'Up3', 'Up_RRCNN3', 'Up2', 'Up_RRCNN2',
                                       'Conv'])
       # self.active = torch.nn.Sigmoid()
       '''
...
def get_r2unet(dataset='pascal_voc', backbone='', pretrained=False, root='../runs/models',
    ...
    model = R2U_Net(datasets[dataset].NUM_CLASS)
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('r2unet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
'''
...
class Attention_block_xxx(nn.Module):
    """
    Attention Block
    """

    def __init__(self, F_g, F_l, F_int):
        super(Attention_block_xxx, self).__init__()
        ...
        self.psi2 = nn.Sequential(
            nn.Conv2d(F_int, 1, kernel_size=(3,1), stride=1, dilation=2,padding=(2,0), bias=True),
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
        self.psi3 = nn.Sequential(
            nn.Conv2d(F_int , 1, kernel_size=(1,3), stride=1, dilation=2,padding=(0,2), bias=True),
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
        self.psi4 = nn.Sequential(
            nn.Conv2d(F_int , 1, kernel_size=3, stride=1, padding=2, dilation=2,bias=True),
            nn.BatchNorm2d(1),
            nn.Sigmoid()
        )
        '''
        ...
        '''
        #self.ratio1 = nn.Parameter(torch.ones(1))
        #self.ratio2 = nn.Parameter(torch.ones(1))
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, g, x):
        ...
        #psi = self.relu(g1 + x1)
        psi = g1+x1
        #psig = self.relu(g1+self.ratio1*x1)
        #psix = self.relu(self.ratio2*g1+x1)
        #psig = self.ratio1*g1+self.ratio2*x1
        #psii = torch.split(psi,[self.cc,self.cc,self.cc,self.cc],dim=1)
        xx = torch.split(x,[self.c,self.c,self.c,self.c],dim=1)
        psi1 = self.psi1(psi)
        psi2 = self.psi2(psi)
        psi3 = self.psi3(psi)
        psi4 = self.psi4(psi)
        ...
        #psi3 = self.psi3(psi)
        #psi4 = self.psi4(psi)
        #xx = torch.split(x, [self.cc, self.cc,self.cc,self.cc], dim=1)
        out1 = xx[0] * psi1
        out2 = xx[1] * psi2
        out3 = xx[2] * psi3
        out4 = xx[3] * psi4
...
class Attention_block_xx(nn.Module):
    """
    Attention Block
    """
#94.777
    def __init__(self, F_g, F_l, F_int):
        super(Attention_block_xx, self).__init__()

        #self.c = F_g//4
        #self.cc = F_int//4
        #'''
        self.W_g = nn.Sequential(
            nn.Conv2d(F_l, F_int, kernel_size=3, stride=1, groups=1, padding=1, bias=True),
            nn.BatchNorm2d(F_int),
            #nn.ReLU(inplace=True)
        )

        self.W_x = nn.Sequential(
            nn.Conv2d(F_g, F_int, kernel_size=3, stride=1, groups=1, padding=1, bias=True),
            nn.BatchNorm2d(F_int),
            #nn.ReLU(inplace=True)
        )
        #'''
        ...
        self.conv2_3 = nn.Sequential(
            nn.Conv2d(F_int, F_int, (1, 3), 1, (0, 1), bias=False),
            nn.BatchNorm2d(F_int)
        )
        self.conv2_4 = nn.Sequential(
            nn.Conv2d(F_int, F_int, (3, 1), 1, (1, 0), bias=False),
            nn.BatchNorm2d(F_int)
        )

        #'''
        self.psi1 = nn.Sequential(
            nn.Conv2d(F_int, 1, kernel_size=1, stride=1, groups=1, padding=0, bias=True),
            nn.BatchNorm2d(1),
            #nn.ReLU(inplace=True)
            nn.Sigmoid()
        )
    ...
    def forward(self, g, x):
        ...
        #out = x*psi1
        _, _, h, w = x.size()
        x2_4 = F.interpolate(self.conv2_3(self.pool3(psi)), (h, w))
        x2_5 = F.interpolate(self.conv2_4(self.pool4(psi)), (h, w))
        # strip pooling的输出结果
        x2 = self.psi1(self.relu(x2_5 + x2_4)) ##Vgg16
        #x2 = self.sigmoid(self.relu(x2_5+x2_4)) ##resnet18
        out = x * x2
        return out


class up_conv_xx(nn.Module):
    """
    Up Convolution Block
    """

    def __init__(self, in_ch, out_ch):
        super(up_conv_xx, self).__init__()
        '''
        self.up = nn.Sequential(
            nn.Upsample(scale_factor=2),
            nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1, bias=True),
...
class AttU_Net(nn.Module):
    ...
    """
    def __init__(self, output_ch=1, img_ch=3 ):
        ...
        self.Conv1 = conv_block_xx(img_ch, filters[0])
        self.Conv2 = conv_block_xx(filters[0], filters[1])
        self.Conv3 = conv_block_xx(filters[1], filters[2])
        self.Conv4 = conv_block_xx(filters[2], filters[3])
        self.Conv5 = conv_block_xx(filters[3], filters[4])
        ...
        '''
        self.__setattr__('exclusive', ['Maxpool1', 'Maxpool2', 'Maxpool3', 'Maxpool4',
                                       'Conv1', 'Conv2', 'Conv3', 'Conv4','Conv5',
                                       'Up5', 'Up_conv5', 'Up4','Up_conv4',
                                       'Up3',  'Up_conv3','Up2','Up_conv2',
                                       'Att5','Att4','Att3','Att2',
                                       'Conv'])
        '''
        #self.active = torch.nn.Sigmoid()
    ...
    def forward(self, x):
        ...
        d5 = torch.cat((e4, d5), dim=1)
        #d5 = torch.cat((e4, d5), dim=1) #hanmh
        d5 = self.Up_conv5(d5)

        d4 = self.Up4(d5)
        e3 = self.Att4(g=d4, x=e3)
        d4 = torch.cat((e3, d4), dim=1)
        #d4 = torch.cat((e3, d4), dim=1)#hanmh
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        e2 = self.Att3(g=d3, x=e2)
        d3 = torch.cat((e2, d3), dim=1)
        #d3 = torch.cat((e2, d3), dim=1) #hanmh
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        e1 = self.Att2(g=d2, x=e1)
        d2 = torch.cat((e1, d2), dim=1) # x1,d2
        #d2 = torch.cat((e1, d2), dim=1)#xiaoxaun
        d2 = self.Up_conv2(d2)
...
def get_AttU_Net(dataset='pascal_voc', backbone='', pretrained=False, root='../runs/models',
               pretrained_base=False, **kwargs):
    acronyms = {
        'pascal_voc': 'pascal_voc',
        'pascal_aug': 'pascal_aug',
    ...
    from core.data.dataloader import datasets
    model = AttU_Net(datasets[dataset].NUM_CLASS)
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('attunet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
class R2AttU_Net(nn.Module):
    ...
    """
    def __init__(self, out_ch=1, in_ch=3,  t=2):
        ...
        '''
        self.__setattr__('exclusive', ['Maxpool1', 'Maxpool2', 'Maxpool3', 'Maxpool4',
                                       'RRCNN1', 'RRCNN2', 'RRCNN3', 'RRCNN4', 'RRCNN5',
                                       'Up5', 'Att5', 'Up_RRCNN5', 'Up4', 'Att4', 'Up_RRCNN4',
                                       'Up3', 'Att3', 'Up_RRCNN3', 'Up2', 'Att2', 'Up_RRCNN2',
                                       'Conv'])
        '''
       # self.active = torch.nn.Sigmoid()
...
def get_r2attunet(dataset='pascal_voc', backbone='', pretrained=False, root='../runs/models',
               pretrained_base=False, **kwargs):
    acronyms = {
        'pascal_voc': 'pascal_voc',
        'pascal_aug': 'pascal_aug',
    ...
    from core.data.dataloader import datasets
    model = R2AttU_Net(datasets[dataset].NUM_CLASS)
    if pretrained:
        from .model_store import get_model_file
        device = torch.device(kwargs['local_rank'])
        model.load_state_dict(torch.load(get_model_file('r2attunet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
