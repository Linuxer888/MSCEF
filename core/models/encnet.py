...
def get_encnet(dataset='pascal_voc', backbone='resnet50', pretrained=False, root='../runs/models',
               pretrained_base=True, **kwargs):
    acronyms = {
    ...
    if pretrained:
        ...
        model.load_state_dict(torch.load(get_model_file('encnet_%s_%s_best_model' % (backbone, acronyms[dataset]), root=root),
                              map_location=device))
    return model
...
def get_encnet_resnet50_laser(**kwargs):
    return get_encnet('laser', 'resnet50', **kwargs)

def get_encnet_resnet101_ade(**kwargs):
    return get_encnet('ade20k', 'resnet101', **kwargs)

def get_encnet_resnet101_laser(**kwargs):
    return get_encnet('laser', 'resnet101', **kwargs)

def get_encnet_resnet152_ade(**kwargs):
    return get_encnet('ade20k', 'resnet152', **kwargs)

def get_encnet_resnet152_laser(**kwargs):
    return get_encnet('laser', 'resnet152', **kwargs)
